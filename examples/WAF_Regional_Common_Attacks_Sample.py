...
SqliMatchSet = t.add_resource(
    SqlInjectionMatchSet(
        ...
        SqlInjectionMatchTuples=[
            ...
            SqlInjectionMatchTuples(
                FieldToMatch=FieldToMatch(Type="BODY"), TextTransformation="URL_DECODE"
            ),
            SqlInjectionMatchTuples(
            ...
            SqlInjectionMatchTuples(
                FieldToMatch=FieldToMatch(Type="URI"), TextTransformation="URL_DECODE"
            ),
        ],
    )
)

SqliRule = t.add_resource(
    Rule(
        "SqliRule",
        Predicates=[
            Predicates(
...
WAFManualIPBlockSet = t.add_resource(
    IPSet(
        "WAFManualIPBlockSet",
...
ManualIPBlockRule = t.add_resource(
    Rule(
        "ManualIPBlockRule",
        Name=Join("", [Ref(WebACLName), "ManualIPBlockRule"]),
        MetricName=Join("", [Ref(WebACLName), "ManualIPBlockRule"]),
        Predicates=[
            Predicates(DataId=Ref(WAFManualIPBlockSet), Type="IPMatch", Negated=False)
        ],
    )
)

SizeMatchSet = t.add_resource(
    SizeConstraintSet(
        "SizeMatchSet",
...
SizeMatchRule = t.add_resource(
    Rule(
        ...
        Predicates=[
            Predicates(DataId=Ref(SizeMatchSet), Type="SizeConstraint", Negated=False)
        ],
    )
)
...
MyWebACL = t.add_resource(
    WebACL(
        ...
        Rules=[
            Rules(
                Action=Action(Type="BLOCK"), Priority=1, RuleId=Ref(ManualIPBlockRule)
            ),
            Rules(Action=Action(Type="COUNT"), Priority=2, RuleId=Ref(SizeMatchRule)),
            Rules(Action=Action(Type="BLOCK"), Priority=3, RuleId=Ref(SqliRule)),
            Rules(Action=Action(Type="BLOCK"), Priority=4, RuleId=Ref(XssRule)),
        ],
