...
deployment_group_ec2 = DeploymentGroup(
    ...
    AutoRollbackConfiguration=auto_rollback_configuration,
    DeploymentStyle=DeploymentStyle(DeploymentOption="WITHOUT_TRAFFIC_CONTROL"),
    ServiceRoleArn="arn:aws:iam::0123456789:role/codedeploy-role",
    Ec2TagSet=Ec2TagSet(
        Ec2TagSetList=[
            Ec2TagSetListObject(
                Ec2TagGroup=[
                    Ec2TagFilters(
                        Key="CodeDeploy", Type="KEY_AND_VALUE", Value="activated"
                    ),
                    Ec2TagFilters(Key="Environment", Type="KEY_AND_VALUE", Value="dev"),
                ]
            )
        ]
    ),
...
deployment_group_on_premises = DeploymentGroup(
    ...
    OnPremisesInstanceTagFilters=[
        OnPremisesInstanceTagFilters(
            Key="Service", Value="DemoApplicationOnPremises", Type="KEY_AND_VALUE"
        )
    ],
)
