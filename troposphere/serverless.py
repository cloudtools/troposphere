        ...
        for k, (key_type, required) in self.props.items():
            if k in kwargs:
                ...
                if not isinstance(kwarg_value, key_type):
                    raise ValueError(f"Provided {self.__class__.__name__} property '{k}' is of type {type(kwarg_value)} instead of expected {key_type}")
            elif required:
                raise ValueError(f"Required {self.__class__.__name__} property '{k}' not provided")

        if remaining_kwargs:
            unexpected_properties = [f"'{k}'" for k in remaining_kwargs.keys()]
            raise ValueError(f"Unexpected {self.__class__.__name__} properties provided: {unexpected_properties}")
