...
class TestIntTypeShouldNotBeUsed(unittest.TestCase):
    ...
    def test_there_should_not_be_any_awsobject_that_uses_int_in_their_fields(self):
        """
        Anything that requires an integer should use `validators.integer` rather than `int`
        `int` does not work with types that behave like an integer (i.e. `long` in Python 2)
        """
    ...
    def _get_all_troposphere_modules(self, dirname):
        return [
            package_name
            for importer, package_name, is_pkg in pkgutil.walk_packages(dirname)
                if not is_pkg and package_name.startswith('troposphere')
        ]
